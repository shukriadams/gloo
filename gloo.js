...
console.log('Running gloo : ' + command);

// verify command
if (!command || allowedCommands.indexOf(command) == -1){
    console.log('Invalid command. Gloo supports : ' + allowedCommands.join(', '));
    return;
}
...
if (command === 'install'){
    ...
    if (!component){
        console.log('gloo install requires a bower name or git url');
        return;
    }
    ...
    if (!component){
        console.log('gloo init requires a bower name');
        return;
    }
    ...
    var watcher = chokidar.watch(config.glooConfig.componentFolder, {
        //ignored: /[\/\\]\./,
        persistent: true
    });
...
*/
    ...
    console.log('gloo : invalid command. Available : ');
    console.log('gloo install name|url :  installs a gloo component via Bower name or git url');
    console.log('gloo dev : builds in dev mode ');
    console.log('gloo release : builds in release mode');
